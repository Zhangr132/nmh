...
public interface AccountInfoMapper extends BaseMapper<AccountInfo> {
    ...
    //新增
    @Insert("insert into account_info values (#{account},#{accName},#{password},#{accPhone},#{isEnable},#{createTime},#{updateTime})")
    int insertAccount(AccountInfo accountInfo);
    //修改
    @Update("update account_info set account=#{account},acc_name=#{accName},password=#{password},acc_phone=#{accPhone},is_enable=#{isEnable} where account=#{account}")
